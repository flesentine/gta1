(() => {
'use strict';
if(window.__gtaRuntime31C3Loaded)return;
window.__gtaRuntime31C3Loaded=true;
const manifest=window.__runtime31C3Manifest;
if(!manifest||!Array.isArray(manifest.modules))throw new Error('Build 31C.3 fresh runtime manifest unavailable');
...
Promise.all(manifest.modules.map(name=>fetch(`${name}?v=31c3fresh1`).then(r=>{
  ...
  try{new Function(source);}catch(err){throw new Error(`Build 31C.3 syntax check failed: ${err.message}`);}
  return eval(source);
}).then(()=>{
  const runtimeDrawPed31C3=drawPed;
  drawPed=function(p){
    if(window.__ped31RenderBitmap&&window.__ped31RenderBitmap(p))return;
    return runtimeDrawPed31C3(p);
  };
  const runtimeDrawCar31C3=drawCar;
  drawCar=function(c){
    if(window.__car31RenderBitmap&&window.__car31RenderBitmap(c))return;
    return runtimeDrawCar31C3(c);
  };
  const title=document.querySelector('#hud .title span');
  if(title)title.textContent='BUILD 31C.3';
}).catch(err=>{
  console.error(err);
  const d=document.getElementById('detail');
  if(d)d.textContent=`BUILD 31C.3 RUNTIME ERROR\n${err.message}`;
  throw err;
});
})();
