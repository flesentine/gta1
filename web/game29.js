...
Promise.all([
  ...
  fetch('runtime29_manifest.json').then(r=>{if(!r.ok)throw new Error(`Unable to load Build 29 manifest (${r.status})`);return r.json();})
  ...
  window.__build29Data=build29;
  window.__runtime29Manifest=manifest;
  ...
  const hook=`\nfetch('runtime29_bundle.js')\n  .then(r=>{if(!r.ok)throw new Error(\`Build 29 runtime bundle (\${r.status})\`);return r.text();})\n  .then(code=>eval(code))\n  .catch(err=>{console.error(err);const d=document.getElementById('detail');if(d)d.textContent=\`BUILD 29 RUNTIME ERROR\\n\${err.message}\`;});\n`;
  ...
  if(detail)detail.textContent=`BUILD 29 LOAD ERROR\n${err.message}`;
});
})();
