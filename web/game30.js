...
Promise.all([
  ...
  const hook=`\nfetch('runtime30_bundle.js')\n  .then(r=>{if(!r.ok)throw new Error(\`Build 30 runtime bundle (\${r.status})\`);return r.text();})\n  .then(code=>eval(code))\n  .catch(err=>{console.error(err);const d=document.getElementById('detail');if(d)d.textContent=\`BUILD 30 RUNTIME ERROR\\n\${err.message}\`;});\n`;
...
}).catch(err=>{console.error(err);const detail=document.getElementById('detail');if(detail)detail.textContent=`BUILD 30 LOAD ERROR\n${err.message}`;});
})();
