(() => {
'use strict';
const BUILD31_VERSION='31c1fresh1';
...
Promise.all([
 ...
 const carPatch=`const CAR31_SEDAN=new Image();
let CAR31_READY=false;
CAR31_SEDAN.decoding='async';
CAR31_SEDAN.addEventListener('load',()=>{CAR31_READY=true;const d=document.getElementById('detail');if(d)d.textContent='31C.1 · TEST SEDAN BITMAP ONLINE · 5 VECTOR / 6 BITMAP';},{once:true});
CAR31_SEDAN.addEventListener('error',()=>{CAR31_READY=false;const d=document.getElementById('detail');if(d)d.textContent='31C.1 · TEST SEDAN LOAD ERROR · VECTOR FALLBACK';},{once:true});
CAR31_SEDAN.src='assets/build31c1/sedan_red.png?v=${BUILD31_VERSION}';
window.__car31RenderBitmap=function(c){
 ...
 ctx.drawImage(CAR31_SEDAN,-20,-36,40,72);
