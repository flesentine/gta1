...
if(!manifest||!Array.isArray(manifest.modules))throw new Error('Build 31A.4 runtime manifest unavailable');
...
Promise.all(manifest.modules.map(name=>fetch(`${name}?v=31a4`).then(r=>{
  ...
  try{new Function(source);}catch(err){throw new Error(`Build 31A.4 syntax check failed: ${err.message}`);}
  return eval(source);
}).then(()=>{
  const title=document.querySelector('#hud .title span');
  if(title)title.textContent='BUILD 31A.4';
}).catch(err=>{
  console.error(err);
  const d=document.getElementById('detail');
  if(d)d.textContent=`BUILD 31A.4 RUNTIME ERROR\n${err.message}`;
  throw err;
});
})();
