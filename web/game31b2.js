(() => {
'use strict';
const BUILD31_VERSION='31b2fresh1';
...
addEventListener('keydown',e=>{
  if(e.code==='F1'){
    ...
    const d=document.getElementById('detail');if(d)d.textContent='31B.2 · VECTOR PLAYER · F2 ALTERNATING-WALK PLAYER';
  }else if(e.code==='F2'){
    e.preventDefault();window.__player31BitmapEnabled=true;
    if(typeof window.__player31ResetGait==='function')window.__player31ResetGait();
    const d=document.getElementById('detail');if(d)d.textContent='31B.2 · ALTERNATING-WALK PLAYER ON · F1 VECTOR PLAYER';
  }
});
addEventListener('keydown',e=>{
  if(e.code==='F3'){
    ...
    const d=document.getElementById('detail');if(d)d.textContent='31B.2 · TEST PED VECTOR · F4 BITMAP PED';
  }else if(e.code==='F4'){
    e.preventDefault();window.__ped31BitmapEnabled=true;
    if(typeof window.__ped31ResetGait==='function')window.__ped31ResetGait();
    const d=document.getElementById('detail');if(d)d.textContent='31B.2 · TEST PED BITMAP ON · F3 VECTOR PED';
  }
});
...
Promise.all([
 ...
 fetch(v('runtime31b2_manifest.json')).then(r=>{if(!r.ok)throw new Error(`Unable to load Build 31B.2 manifest (${r.status})`);return r.json();})
]).then(([core,city,harbor,west,missions,build29,manifest])=>{
 window.__city28Data=city;window.__harbor18Data=harbor;window.__west25Data=west;window.__missions28Data=missions;window.__build29Data=build29;window.__runtime31B2Manifest=manifest;
 ...
 if(!core.includes(playerNeedle))throw new Error('Build 31B.2 player draw marker missing');
...
function player31Load(img,path){
 img.decoding='async';
 img.addEventListener('load',()=>{PLAYER31_READY++;if(PLAYER31_READY===3){const d=document.getElementById('detail');if(d)d.textContent='31B.2 · 3-FRAME ALTERNATING WALK ONLINE · F1 VECTOR / F2 BITMAP';}},{once:true});
 img.addEventListener('error',()=>{const d=document.getElementById('detail');if(d)d.textContent='31B.2 · REQUIRED PLAYER FRAME LOAD ERROR · VECTOR FALLBACK';},{once:true});
 img.src=path;
}
...
function drawPlayer(){
 ...
 if(!core.includes(pedSpawnNeedle))throw new Error('Build 31B.2 pedestrian spawn marker missing');
 core=core.replace(pedSpawnNeedle,pedSpawnNeedle+"const ped31RouteA=[[102,102],[798,102],[798,548],[102,548]],ped31RouteB=[[-102,102],[-798,102],[-798,548],[-102,548]];Object.assign(peds[0],{x:102,y:102,route:ped31RouteA,index:1,color:'#d64a40',speed:52,panic:0,down:0,dead:0,fx:1,fy:0,behavior16:'BITMAP_TEST',bitmapTest31B2:true,bitmapSet31B2:0});Object.assign(peds[1],{x:-102,y:102,route:ped31RouteB,index:1,color:'#2fa3ad',speed:52,panic:0,down:0,dead:0,fx:-1,fy:0,behavior16:'BITMAP_TEST',bitmapTest31B2:true,bitmapSet31B2:1});");

 const pedStart=core.indexOf("function drawPed(p){");
 const pedEnd=core.indexOf("\nfunction drawPickup",pedStart);
 if(pedStart<0||pedEnd<0)throw new Error('Build 31B.2 pedestrian draw marker missing');
...
function ped31Load(setIndex,img,path){
 img.decoding='async';
 img.addEventListener('load',()=>{const set=PED31_SETS[setIndex];set.ready++;if(set.ready===3){const d=document.getElementById('detail');if(d)d.textContent='31B.2 · BITMAP PED SET '+(setIndex+1)+' ONLINE · F3 VECTOR / F4 BITMAP';}},{once:true});
 img.addEventListener('error',()=>{const d=document.getElementById('detail');if(d)d.textContent='31B.2 · PEDESTRIAN SET '+(setIndex+1)+' FRAME LOAD ERROR · PER-SET FALLBACK';},{once:true});
 img.src=path;
}
...
window.__ped31RenderBitmap=function(p){
 if(!p||!p.bitmapTest31B2||p.dead>0||p.down>0||window.__ped31BitmapEnabled===false)return false;
 const setIndex=Number.isInteger(p.bitmapSet31B2)?p.bitmapSet31B2:0;
...
function drawPed(p){
 ...
 const hook=`\nfetch('runtime31b2_bundle.js?v=${BUILD31_VERSION}')\n .then(r=>{if(!r.ok)throw new Error(\`Build 31B.2 runtime bundle (\${r.status})\`);return r.text();})\n .then(code=>eval(code))\n .catch(err=>{console.error(err);const d=document.getElementById('detail');if(d)d.textContent=\`BUILD 31B.2 RUNTIME ERROR\\n\${err.message}\`;});\n`;
 ...
 console.error(err);const d=document.getElementById('detail');if(d)d.textContent=`BUILD 31B.2 LOAD ERROR\n${err.message}`;
});
})();
