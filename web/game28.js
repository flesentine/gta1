...
Promise.all([
  ...
  fetch('runtime28_manifest.json').then(r=>{if(!r.ok)throw new Error(`Unable to load Build 28 manifest (${r.status})`);return r.json();})
]).then(([core,city,harbor,west,missions,manifest])=>{
  window.__city28Data=city;
  window.__harbor18Data=harbor;
  window.__west25Data=west;
  window.__missions28Data=missions;
  window.__runtime28Manifest=manifest;
  ...
  const hook=`\nfetch('runtime28_bundle.js')\n  .then(r=>{if(!r.ok)throw new Error(\`Build 28 runtime bundle (\${r.status})\`);return r.text();})\n  .then(code=>eval(code))\n  .catch(err=>{console.error(err);const d=document.getElementById('detail');if(d)d.textContent=\`BUILD 28 RUNTIME ERROR\\n\${err.message}\`;});\n`;
  ...
  if(detail)detail.textContent=`BUILD 28 LOAD ERROR\n${err.message}`;
});
})();
