(() => {
'use strict';
const BUILD31_VERSION='31b1afresh1';
...
addEventListener('keydown',e=>{
  if(e.code==='F1'){
    ...
    const d=document.getElementById('detail');if(d)d.textContent='31B.1a · VECTOR PLAYER · F2 ALTERNATING-WALK PLAYER';
  }else if(e.code==='F2'){
    e.preventDefault();window.__player31BitmapEnabled=true;
    if(typeof window.__player31ResetGait==='function')window.__player31ResetGait();
    const d=document.getElementById('detail');if(d)d.textContent='31B.1a · ALTERNATING-WALK PLAYER ON · F1 VECTOR PLAYER';
  }
});
addEventListener('keydown',e=>{
  if(e.code==='F3'){
    ...
    const d=document.getElementById('detail');if(d)d.textContent='31B.1a · TEST PED VECTOR · F4 BITMAP PED';
  }else if(e.code==='F4'){
    e.preventDefault();window.__ped31BitmapEnabled=true;
    if(typeof window.__ped31ResetGait==='function')window.__ped31ResetGait();
    const d=document.getElementById('detail');if(d)d.textContent='31B.1a · TEST PED BITMAP ON · F3 VECTOR PED';
  }
});
...
Promise.all([
 ...
 fetch(v('runtime31b1a_manifest.json')).then(r=>{if(!r.ok)throw new Error(`Unable to load Build 31B.1a manifest (${r.status})`);return r.json();})
]).then(([core,city,harbor,west,missions,build29,manifest])=>{
 window.__city28Data=city;window.__harbor18Data=harbor;window.__west25Data=west;window.__missions28Data=missions;window.__build29Data=build29;window.__runtime31B1AManifest=manifest;
 ...
 if(!core.includes(playerNeedle))throw new Error('Build 31B.1a player draw marker missing');
...
function player31Load(img,path){
 img.decoding='async';
 img.addEventListener('load',()=>{PLAYER31_READY++;if(PLAYER31_READY===3){const d=document.getElementById('detail');if(d)d.textContent='31B.1a · 3-FRAME ALTERNATING WALK ONLINE · F1 VECTOR / F2 BITMAP';}},{once:true});
 img.addEventListener('error',()=>{const d=document.getElementById('detail');if(d)d.textContent='31B.1a · REQUIRED PLAYER FRAME LOAD ERROR · VECTOR FALLBACK';},{once:true});
 img.src=path;
}
...
function drawPlayer(){
 ...
 if(!core.includes(pedSpawnNeedle))throw new Error('Build 31B.1a pedestrian spawn marker missing');
 core=core.replace(pedSpawnNeedle,pedSpawnNeedle+"const ped31Route=[[102,102],[798,102],[798,548],[102,548]];Object.assign(peds[0],{x:102,y:102,route:ped31Route,index:1,color:'#d64a40',speed:52,panic:0,down:0,dead:0,fx:1,fy:0,behavior16:'BITMAP_TEST',bitmapTest31B1a:true});");

 const pedStart=core.indexOf("function drawPed(p){");
 const pedEnd=core.indexOf("\nfunction drawPickup",pedStart);
 if(pedStart<0||pedEnd<0)throw new Error('Build 31B.1a pedestrian draw marker missing');
...
let PED31_READY=0,PED31_HAVE_LAST=false,PED31_LAST_X=0,PED31_LAST_Y=0,PED31_ANIM_DIST=0,PED31_WAS_MOVING=false,PED31_LEAD_FOOT=1,PED31_FACE_X=0,PED31_FACE_Y=1,PED31_TRACKED=null;
function ped31Load(img,path){
 img.decoding='async';
 img.addEventListener('load',()=>{PED31_READY++;if(PED31_READY===3){const d=document.getElementById('detail');if(d)d.textContent='31B.1a · ONE BITMAP PEDESTRIAN ONLINE · F3 VECTOR / F4 BITMAP';}},{once:true});
 img.addEventListener('error',()=>{const d=document.getElementById('detail');if(d)d.textContent='31B.1a · PEDESTRIAN FRAME LOAD ERROR · VECTOR FALLBACK';},{once:true});
 img.src=path;
}
ped31Load(PED31_IDLE,PED31_PATHS[0]);
ped31Load(PED31_STEP_A,PED31_PATHS[1]);
ped31Load(PED31_STEP_B,PED31_PATHS[2]);
function ped31ResetGait(){PED31_HAVE_LAST=false;PED31_ANIM_DIST=0;PED31_WAS_MOVING=false;PED31_TRACKED=null;}
...
function ped31DrawBitmap(p){
 if(PED31_TRACKED!==p){ped31ResetGait();PED31_TRACKED=p;PED31_LAST_X=p.x;PED31_LAST_Y=p.y;PED31_HAVE_LAST=true;}
 let moved=0;
...
window.__ped31RenderBitmap=function(p){
 if(!p||!p.bitmapTest31B1a||p.dead>0||p.down>0||window.__ped31BitmapEnabled===false||PED31_READY!==3)return false;
 ped31DrawBitmap(p);return true;
};
function drawPed(p){
 ...
 const hook=`\nfetch('runtime31b1a_bundle.js?v=${BUILD31_VERSION}')\n .then(r=>{if(!r.ok)throw new Error(\`Build 31B.1a runtime bundle (\${r.status})\`);return r.text();})\n .then(code=>eval(code))\n .catch(err=>{console.error(err);const d=document.getElementById('detail');if(d)d.textContent=\`BUILD 31B.1a RUNTIME ERROR\\n\${err.message}\`;});\n`;
 ...
 console.error(err);const d=document.getElementById('detail');if(d)d.textContent=`BUILD 31B.1a LOAD ERROR\n${err.message}`;
});
})();
