...
addEventListener('keydown',e=>{
  if(e.code==='F1'){
    ...
    const d=document.getElementById('detail');if(d)d.textContent='31A.3 · VECTOR PLAYER · F2 ANIMATED PLAYER';
  }else if(e.code==='F2'){
    e.preventDefault();window.__player31BitmapEnabled=true;
    const d=document.getElementById('detail');if(d)d.textContent='31A.3 · ANIMATED PLAYER ON · F1 VECTOR PLAYER';
  }
});
Promise.all([
 ...
 fetch('runtime31_manifest.json').then(r=>{if(!r.ok)throw new Error(`Unable to load Build 31A.3 manifest (${r.status})`);return r.json();})
 ...
 if(!core.includes(playerNeedle))throw new Error('Build 31A.3 player draw marker missing');
 const playerPatch=`const PLAYER31_IDLE=new Image();
const PLAYER31_WALK=[new Image(),new Image(),new Image(),new Image()];
const PLAYER31_PATHS=['assets/build31a/player_idle.png','assets/build31a/player_walk_0.png','assets/build31a/player_walk_1.png','assets/build31a/player_walk_2.png','assets/build31a/player_walk_3.png'];
let PLAYER31_READY=0;
function player31Load(img,path){
 img.decoding='async';
 img.addEventListener('load',()=>{PLAYER31_READY++;if(PLAYER31_READY===5){const d=document.getElementById('detail');if(d)d.textContent='31A.3 · 4-FRAME WALK ONLINE · F1 VECTOR / F2 ANIMATED';}},{once:true});
 img.addEventListener('error',()=>{const d=document.getElementById('detail');if(d)d.textContent='31A.3 · PLAYER FRAME LOAD ERROR · VECTOR FALLBACK';},{once:true});
 img.src=path;
}
...
function drawPlayer(){
 ...
 const hook=`\nfetch('runtime31_bundle.js')\n .then(r=>{if(!r.ok)throw new Error(\`Build 31A.3 runtime bundle (\${r.status})\`);return r.text();})\n .then(code=>eval(code))\n .catch(err=>{console.error(err);const d=document.getElementById('detail');if(d)d.textContent=\`BUILD 31A.3 RUNTIME ERROR\\n\${err.message}\`;});\n`;
 ...
 console.error(err);const d=document.getElementById('detail');if(d)d.textContent=`BUILD 31A.3 LOAD ERROR\n${err.message}`;
});
})();
