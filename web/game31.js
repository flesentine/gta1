(() => {
'use strict';
const BUILD31_VERSION='31a5';
window.__player31BitmapEnabled=true;
addEventListener('keydown',e=>{
  if(e.code==='F1'){
    e.preventDefault();window.__player31BitmapEnabled=false;
    const d=document.getElementById('detail');if(d)d.textContent='31A.5 · VECTOR PLAYER · F2 HARDENED PLAYER';
  }else if(e.code==='F2'){
    e.preventDefault();window.__player31BitmapEnabled=true;
    const d=document.getElementById('detail');if(d)d.textContent='31A.5 · HARDENED PLAYER ON · F1 VECTOR PLAYER';
  }
});
...
Promise.all([
 ...
 fetch(v('runtime31_manifest.json')).then(r=>{if(!r.ok)throw new Error(`Unable to load Build 31A.5 manifest (${r.status})`);return r.json();})
 ...
 if(!core.includes(playerNeedle))throw new Error('Build 31A.5 player draw marker missing');
...
let PLAYER31_READY=0,PLAYER31_HAVE_LAST=false,PLAYER31_LAST_X=0,PLAYER31_LAST_Y=0,PLAYER31_ANIM_DIST=0;
function player31Load(img,path){
 img.decoding='async';
 img.addEventListener('load',()=>{PLAYER31_READY++;if(PLAYER31_READY===5){const d=document.getElementById('detail');if(d)d.textContent='31A.5 · HARD PIXELS + DISTANCE WALK ONLINE · F1 VECTOR / F2 BITMAP';}},{once:true});
 img.addEventListener('error',()=>{const d=document.getElementById('detail');if(d)d.textContent='31A.5 · PLAYER FRAME LOAD ERROR · VECTOR FALLBACK';},{once:true});
 img.src=path;
}
...
function drawPlayer(){
 ...
 const frame=moving?PLAYER31_WALK[Math.floor(PLAYER31_ANIM_DIST/11)%PLAYER31_WALK.length]:PLAYER31_IDLE;
 ...
 const hook=`\nfetch('runtime31_bundle.js?v=${BUILD31_VERSION}')\n .then(r=>{if(!r.ok)throw new Error(\`Build 31A.5 runtime bundle (\${r.status})\`);return r.text();})\n .then(code=>eval(code))\n .catch(err=>{console.error(err);const d=document.getElementById('detail');if(d)d.textContent=\`BUILD 31A.5 RUNTIME ERROR\\n\${err.message}\`;});\n`;
 ...
 console.error(err);const d=document.getElementById('detail');if(d)d.textContent=`BUILD 31A.5 LOAD ERROR\n${err.message}`;
});
})();
