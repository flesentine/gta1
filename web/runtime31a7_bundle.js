(() => {
'use strict';
if(window.__gtaRuntime31A7Loaded)return;
window.__gtaRuntime31A7Loaded=true;
const manifest=window.__runtime31A7Manifest;
if(!manifest||!Array.isArray(manifest.modules))throw new Error('Build 31A.7 fresh runtime manifest unavailable');
...
Promise.all(manifest.modules.map(name=>fetch(`${name}?v=31a7fresh1`).then(r=>{
  ...
  try{new Function(source);}catch(err){throw new Error(`Build 31A.7 syntax check failed: ${err.message}`);}
  return eval(source);
}).then(()=>{
  const title=document.querySelector('#hud .title span');
  if(title)title.textContent='BUILD 31A.7';
}).catch(err=>{
  console.error(err);
  const d=document.getElementById('detail');
  if(d)d.textContent=`BUILD 31A.7 RUNTIME ERROR\n${err.message}`;
  throw err;
});
})();
