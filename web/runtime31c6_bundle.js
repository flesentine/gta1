(() => {
'use strict';
if(window.__gtaRuntime31C6Loaded)return;
window.__gtaRuntime31C6Loaded=true;
const manifest=window.__runtime31C6Manifest;
if(!manifest||!Array.isArray(manifest.modules))throw new Error('Build 31C.6 fresh runtime manifest unavailable');
...
Promise.all(manifest.modules.map(name=>fetch(`${name}?v=31c6fresh1`).then(r=>{
  ...
  try{new Function(source);}catch(err){throw new Error(`Build 31C.6 syntax check failed: ${err.message}`);}
  return eval(source);
}).then(()=>{
  const runtimeDrawPed31C6=drawPed;
  drawPed=function(p){
    if(window.__ped31RenderBitmap&&window.__ped31RenderBitmap(p))return;
    return runtimeDrawPed31C6(p);
  };
  const runtimeDrawCar31C6=drawCar;
  drawCar=function(c){
    if(window.__car31RenderBitmap&&window.__car31RenderBitmap(c))return;
    return runtimeDrawCar31C6(c);
  };
  const title=document.querySelector('#hud .title span');
  if(title)title.textContent='BUILD 31C.6';
}).catch(err=>{
  console.error(err);
  const d=document.getElementById('detail');
  if(d)d.textContent=`BUILD 31C.6 RUNTIME ERROR\n${err.message}`;
  throw err;
});
})();
