(() => {
'use strict';
if(window.__gtaRuntime31B5Loaded)return;
window.__gtaRuntime31B5Loaded=true;
const manifest=window.__runtime31B5Manifest;
if(!manifest||!Array.isArray(manifest.modules))throw new Error('Build 31B.5 fresh runtime manifest unavailable');
...
Promise.all(manifest.modules.map(name=>fetch(`${name}?v=31b5fresh1`).then(r=>{
  ...
  try{new Function(source);}catch(err){throw new Error(`Build 31B.5 syntax check failed: ${err.message}`);}
  return eval(source);
}).then(()=>{
  const runtimeDrawPed31B5=drawPed;
  drawPed=function(p){
    if(window.__ped31RenderBitmap&&window.__ped31RenderBitmap(p))return;
    return runtimeDrawPed31B5(p);
  };
  const title=document.querySelector('#hud .title span');
  if(title)title.textContent='BUILD 31B.5';
}).catch(err=>{
  console.error(err);
  const d=document.getElementById('detail');
  if(d)d.textContent=`BUILD 31B.5 RUNTIME ERROR\n${err.message}`;
  throw err;
});
})();
