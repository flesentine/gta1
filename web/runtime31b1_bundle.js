(() => {
'use strict';
if(window.__gtaRuntime31B1Loaded)return;
window.__gtaRuntime31B1Loaded=true;
const manifest=window.__runtime31B1Manifest;
if(!manifest||!Array.isArray(manifest.modules))throw new Error('Build 31B.1 fresh runtime manifest unavailable');
...
Promise.all(manifest.modules.map(name=>fetch(`${name}?v=31b1fresh1`).then(r=>{
  ...
  try{new Function(source);}catch(err){throw new Error(`Build 31B.1 syntax check failed: ${err.message}`);}
  return eval(source);
}).then(()=>{
  const title=document.querySelector('#hud .title span');
  if(title)title.textContent='BUILD 31B.1';
}).catch(err=>{
  console.error(err);
  const d=document.getElementById('detail');
  if(d)d.textContent=`BUILD 31B.1 RUNTIME ERROR\n${err.message}`;
  throw err;
});
})();
