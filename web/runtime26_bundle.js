...
Promise.all(modules.map(name => fetch(name).then(r => {
  ...
  catch (err) { throw new Error(`ordered Build 26 runtime bundle syntax: ${err.message}`); }
  eval(source);
}).catch(err => {
  console.error(err);
  const detail = document.getElementById('detail');
  if (detail) detail.textContent = `BUILD 26 RUNTIME BUNDLE ERROR\n${err.message}`;
});
})();
