(() => {
'use strict';
if(window.__gtaRuntime31C1Loaded)return;
window.__gtaRuntime31C1Loaded=true;
const manifest=window.__runtime31C1Manifest;
if(!manifest||!Array.isArray(manifest.modules))throw new Error('Build 31C.1 fresh runtime manifest unavailable');
...
Promise.all(manifest.modules.map(name=>fetch(`${name}?v=31c1fresh5`).then(r=>{
  ...
  try{new Function(source);}catch(err){throw new Error(`Build 31C.1 syntax check failed: ${err.message}`);}
  return eval(source);
}).then(()=>{
  const runtimeDrawPed31C1=drawPed;
  drawPed=function(p){
    if(window.__ped31RenderBitmap&&window.__ped31RenderBitmap(p))return;
    return runtimeDrawPed31C1(p);
  };
  const runtimeDrawCar31C1=drawCar;
  drawCar=function(c){
    if(window.__car31RenderBitmap&&window.__car31RenderBitmap(c))return;
    return runtimeDrawCar31C1(c);
  };
  const title=document.querySelector('#hud .title span');
  if(title)title.textContent='BUILD 31C.1';
}).catch(err=>{
  console.error(err);
  const d=document.getElementById('detail');
  if(d)d.textContent=`BUILD 31C.1 RUNTIME ERROR\n${err.message}`;
  throw err;
});
})();
