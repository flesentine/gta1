(() => {
'use strict';
if(window.__gtaRuntime29Loaded)return;
window.__gtaRuntime29Loaded=true;

const manifest=window.__runtime29Manifest;
if(!manifest||!Array.isArray(manifest.modules))throw new Error('Build 29 runtime manifest unavailable');

function flattenModule29(name,source){
...
Promise.all(manifest.modules.map(name=>fetch(name).then(r=>{
  ...
  const source=parts.map(p=>flattenModule29(p.name,p.source)).join('\n');
  try{new Function(source);}catch(err){throw new Error(`Flattened runtime syntax check failed: ${err.message}`);}
  eval(source);
}).catch(err=>{
  console.error(err);
  const detail=document.getElementById('detail');
  if(detail)detail.textContent=`BUILD 29 BUNDLE ERROR\n${err.message}`;
  throw err;
});
})();
