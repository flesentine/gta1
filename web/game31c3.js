(() => {
'use strict';
const BUILD31_VERSION='31c3fresh1';
...
addEventListener('keydown',e=>{
  if(e.code==='F1'){
    ...
    const d=document.getElementById('detail');if(d)d.textContent='31C.3 · VECTOR PLAYER · F2 ALTERNATING-WALK PLAYER';
  }else if(e.code==='F2'){
    e.preventDefault();window.__player31BitmapEnabled=true;
    if(typeof window.__player31ResetGait==='function')window.__player31ResetGait();
    const d=document.getElementById('detail');if(d)d.textContent='31C.3 · ALTERNATING-WALK PLAYER ON · F1 VECTOR PLAYER';
  }
});
addEventListener('keydown',e=>{
  if(e.code==='F3'){
    ...
    const d=document.getElementById('detail');if(d)d.textContent='31C.3 · BITMAP CROWD VECTOR · F4 BITMAP PED';
  }else if(e.code==='F4'){
    e.preventDefault();window.__ped31BitmapEnabled=true;
    if(typeof window.__ped31ResetGait==='function')window.__ped31ResetGait();
    const d=document.getElementById('detail');if(d)d.textContent='31C.3 · BITMAP CROWD ON · F3 VECTOR PED';
  }
});
addEventListener('keydown',e=>{
  if(e.code==='Digit5'){
    e.preventDefault();window.__car31BitmapEnabled=false;
    const d=document.getElementById('detail');if(d)d.textContent='31C.3 · VECTOR TEST VEHICLES · 6 BITMAP VEHICLES';
  }else if(e.code==='Digit6'){
    e.preventDefault();window.__car31BitmapEnabled=true;
    const d=document.getElementById('detail');if(d)d.textContent='31C.3 · TWO-SILHOUETTE BITMAP VEHICLES ON · 5 VECTOR VEHICLES';
  }
});
...
Promise.all([
 ...
 fetch(v('runtime31c3_manifest.json')).then(r=>{if(!r.ok)throw new Error(`Unable to load Build 31C.3 manifest (${r.status})`);return r.json();})
]).then(([core,city,harbor,west,missions,build29,manifest])=>{
 window.__city28Data=city;window.__harbor18Data=harbor;window.__west25Data=west;window.__missions28Data=missions;window.__build29Data=build29;window.__runtime31C3Manifest=manifest;
 ...
 if(!core.includes(carSpawnNeedle))throw new Error('Build 31C.3 vehicle spawn marker missing');
 core=core.replace(carSpawnNeedle,carSpawnNeedle+"if(cars[1]){cars[1].bitmapVehicle31C3=true;cars[1].bitmapVehicleSprite31C3=0;}if(cars[2]){cars[2].bitmapVehicle31C3=true;cars[2].bitmapVehicleSprite31C3=1;}");

 const carStart=core.indexOf("function drawCar(c){");
 const carEnd=core.indexOf("\nfunction drawCop",carStart);
 if(carStart<0||carEnd<0)throw new Error('Build 31C.3 vehicle draw marker missing');
...
function car31Load(sprite){
 ...
 img.addEventListener('load',()=>{sprite.ready=true;const d=document.getElementById('detail');if(d)d.textContent='31C.3 · '+sprite.label+' ONLINE · TWO VEHICLE SILHOUETTES';},{once:true});
 img.addEventListener('error',()=>{sprite.ready=false;const d=document.getElementById('detail');if(d)d.textContent='31C.3 · '+sprite.label+' LOAD ERROR · PER-CAR VECTOR FALLBACK';},{once:true});
 img.src=sprite.path;
}
CAR31_SPRITES.forEach(car31Load);
window.__car31Sprites31C3=CAR31_SPRITES;
window.__car31RenderBitmap=function(c){
 if(!c||!c.bitmapVehicle31C3||window.__car31BitmapEnabled===false)return false;
 const spriteIndex=Number.isInteger(c.bitmapVehicleSprite31C3)?c.bitmapVehicleSprite31C3:0;
 ...
 if(!core.includes(playerNeedle))throw new Error('Build 31C.3 player draw marker missing');
...
function player31Load(img,path){
 img.decoding='async';
 img.addEventListener('load',()=>{PLAYER31_READY++;if(PLAYER31_READY===3){const d=document.getElementById('detail');if(d)d.textContent='31C.3 · 3-FRAME ALTERNATING WALK ONLINE · F1 VECTOR / F2 BITMAP';}},{once:true});
 img.addEventListener('error',()=>{const d=document.getElementById('detail');if(d)d.textContent='31C.3 · REQUIRED PLAYER FRAME LOAD ERROR · VECTOR FALLBACK';},{once:true});
 img.src=path;
}
...
function drawPlayer(){
 ...
 if(!core.includes(pedSpawnNeedle))throw new Error('Build 31C.3 pedestrian spawn marker missing');
 ...
 if(pedStart<0||pedEnd<0)throw new Error('Build 31C.3 pedestrian draw marker missing');
...
function ped31Load(setIndex,img,path){
 img.decoding='async';
 img.addEventListener('load',()=>{const set=PED31_SETS[setIndex];set.ready++;if(set.ready===3){const d=document.getElementById('detail');if(d)d.textContent='31C.3 · BITMAP CROWD SET '+(setIndex+1)+' ONLINE · F3 VECTOR / F4 BITMAP';}},{once:true});
 img.addEventListener('error',()=>{const d=document.getElementById('detail');if(d)d.textContent='31C.3 · BITMAP CROWD SET '+(setIndex+1)+' FRAME LOAD ERROR · PER-SET FALLBACK';},{once:true});
 img.src=path;
}
...
function drawPed(p){
 ...
 const hook=`\nfetch('runtime31c3_bundle.js?v=${BUILD31_VERSION}')\n .then(r=>{if(!r.ok)throw new Error(\`Build 31C.3 runtime bundle (\${r.status})\`);return r.text();})\n .then(code=>eval(code))\n .catch(err=>{console.error(err);const d=document.getElementById('detail');if(d)d.textContent=\`BUILD 31C.3 RUNTIME ERROR\\n\${err.message}\`;});\n`;
 ...
 console.error(err);const d=document.getElementById('detail');if(d)d.textContent=`BUILD 31C.3 LOAD ERROR\n${err.message}`;
});
})();
