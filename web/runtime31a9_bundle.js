(() => {
'use strict';
if(window.__gtaRuntime31A9Loaded)return;
window.__gtaRuntime31A9Loaded=true;
const manifest=window.__runtime31A9Manifest;
if(!manifest||!Array.isArray(manifest.modules))throw new Error('Build 31A.9 fresh runtime manifest unavailable');
...
Promise.all(manifest.modules.map(name=>fetch(`${name}?v=31a9fresh1`).then(r=>{
  ...
  try{new Function(source);}catch(err){throw new Error(`Build 31A.9 syntax check failed: ${err.message}`);}
  return eval(source);
}).then(()=>{
  const title=document.querySelector('#hud .title span');
  if(title)title.textContent='BUILD 31A.9';
}).catch(err=>{
  console.error(err);
  const d=document.getElementById('detail');
  if(d)d.textContent=`BUILD 31A.9 RUNTIME ERROR\n${err.message}`;
  throw err;
});
})();
