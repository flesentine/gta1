(() => {
'use strict';
const BUILD31_VERSION='31a6fresh2';
window.__player31BitmapEnabled=true;
addEventListener('keydown',e=>{
  if(e.code==='F1'){
    e.preventDefault();window.__player31BitmapEnabled=false;
    const d=document.getElementById('detail');if(d)d.textContent='31A.6 · VECTOR PLAYER · F2 HARDENED PLAYER';
  }else if(e.code==='F2'){
    e.preventDefault();window.__player31BitmapEnabled=true;
    const d=document.getElementById('detail');if(d)d.textContent='31A.6 · HARDENED PLAYER ON · F1 VECTOR PLAYER';
  }
});
...
Promise.all([
 ...
 fetch(v('runtime31a6_manifest.json')).then(r=>{if(!r.ok)throw new Error(`Unable to load Build 31A.6 fresh manifest (${r.status})`);return r.json();})
]).then(([core,city,harbor,west,missions,build29,manifest])=>{
 window.__city28Data=city;window.__harbor18Data=harbor;window.__west25Data=west;window.__missions28Data=missions;window.__build29Data=build29;window.__runtime31A6Manifest=manifest;
 ...
 if(!core.includes(playerNeedle))throw new Error('Build 31A.6 player draw marker missing');
...
function player31Load(img,path){
 img.decoding='async';
 img.addEventListener('load',()=>{PLAYER31_READY++;if(PLAYER31_READY===5){const d=document.getElementById('detail');if(d)d.textContent='31A.6 · CARDINAL WALK ONLINE · F1 VECTOR / F2 BITMAP';}},{once:true});
 img.addEventListener('error',()=>{const d=document.getElementById('detail');if(d)d.textContent='31A.6 · PLAYER FRAME LOAD ERROR · VECTOR FALLBACK';},{once:true});
 img.src=path;
}
player31Load(PLAYER31_IDLE,PLAYER31_PATHS[0]);
PLAYER31_WALK.forEach((img,i)=>player31Load(img,PLAYER31_PATHS[i+1]));
function player31UpdateFacing(){
 const fx=Number.isFinite(player.fx)?player.fx:0,fy=Number.isFinite(player.fy)?player.fy:-1;
...
function drawPlayer(){
 ...
 if(moving){PLAYER31_ANIM_DIST+=moved;player31UpdateFacing();}else PLAYER31_ANIM_DIST=0;
 const frame=moving?PLAYER31_WALK[Math.floor(PLAYER31_ANIM_DIST/19)%PLAYER31_WALK.length]:PLAYER31_IDLE;
 ...
 const hook=`\nfetch('runtime31a6_bundle.js?v=${BUILD31_VERSION}')\n .then(r=>{if(!r.ok)throw new Error(\`Build 31A.6 fresh runtime bundle (\${r.status})\`);return r.text();})\n .then(code=>eval(code))\n .catch(err=>{console.error(err);const d=document.getElementById('detail');if(d)d.textContent=\`BUILD 31A.6 RUNTIME ERROR\\n\${err.message}\`;});\n`;
 ...
 console.error(err);const d=document.getElementById('detail');if(d)d.textContent=`BUILD 31A.6 LOAD ERROR\n${err.message}`;
});
})();
