extends "res://scripts/mission_terminal_build18.gd"

func _tag_build_14() -> void:
    if director != null and director.hud_label != null:
        director.hud_label.text = director.hud_label.text.replace("BUILD 18", "BUILD 19")
...
func _populate_menu() -> void:
    ...
    panel.offset_top = -435.0
    panel.offset_right = 350.0
    panel.offset_bottom = 435.0
    panel.color = Color(0.04, 0.055, 0.07, 0.98)
    panel.mouse_filter = Control.MOUSE_FILTER_STOP
    overlay.add_child(panel)
    var title := Label.new()
    title.position = Vector2(24, 10)
    title.size = Vector2(652, 54)
    title.text = "BUILD 19 — MISSION TERMINAL\nCHOOSE A JOB"
    ...
    for i in range(director.campaign.size()):
        ...
        button.size = Vector2(652, 46)
        var reward := int(mission.get("base_reward", 0))
        if i < unlocked:
            button.text = "%d. %s    %s    BASE %d" % [i + 1, str(mission.get("title", "MISSION")), _type_label(str(mission.get("type", "mission"))), reward]
        ...
        y += 49.0
    var help := Label.new()
    help.position = Vector2(24, 520)
    help.size = Vector2(652, 80)
    help.text = "1–9: select    Esc: close\nNIGHT SHIFT is a four-stop Docklands pressure run."
