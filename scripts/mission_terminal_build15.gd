extends "res://scripts/mission_terminal_build14.gd"

func _tag_build_14() -> void:
    if director != null and director.hud_label != null:
        director.hud_label.text = director.hud_label.text.replace("BUILD 9", "BUILD 15").replace("BUILD 11", "BUILD 15").replace("BUILD 12", "BUILD 15").replace("BUILD 13", "BUILD 15").replace("BUILD 14", "BUILD 15")
...
func _populate_menu() -> void:
    ...
    title.size = Vector2(592, 62)
    title.text = "BUILD 15 — MISSION TERMINAL\nCHOOSE A JOB"
    ...
    var help := Label.new()
    help.position = Vector2(24, 464)
    help.size = Vector2(592, 64)
    help.text = "1–6: select    Esc: close\nCROSSTOWN + DEAD DROP + RED FLAG unlock after clearing the core level."
    help.add_theme_font_size_override("font_size", 14)
    panel.add_child(help)

func _type_label(value: String) -> String:
    if value == "character_target":
        return "TARGET + POLICE ESCAPE"
