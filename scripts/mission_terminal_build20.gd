extends "res://scripts/mission_terminal_build19.gd"

func _tag_build_14() -> void:
    if director != null and director.hud_label != null:
        director.hud_label.text = director.hud_label.text.replace("BUILD 19", "BUILD 20")
...
func _populate_menu() -> void:
    ...
    title.size = Vector2(672, 54)
    title.text = "BUILD 20 — MISSION TERMINAL\nCHOOSE A JOB"
    ...
    for i in range(director.campaign.size()):
        ...
        button.size = Vector2(672, 42)
        var reward := int(mission.get("base_reward", 0))
        var bonus := int(mission.get("bonus_reward", 0))
        var bonus_text := " + BONUS %d" % bonus if bonus > 0 else ""
        if i < unlocked:
            button.text = "%d. %s    %s    BASE %d%s" % [
                i + 1, str(mission.get("title", "MISSION")),
                _type_label(str(mission.get("type", "mission"))), reward, bonus_text
            ]
    ...
    var help := Label.new()
    help.position = Vector2(24, 530)
    help.size = Vector2(672, 86)
    help.text = "1–9, 0: select    Esc: close\nGREEN WAVE: finish normally, or keep the clean-driving bonus by not blasting red lights."
    help.add_theme_font_size_override("font_size", 14)
    panel.add_child(help)

func _type_label(value: String) -> String:
    if value == "bonus_checkpoint_run":
        return "SIGNAL RUN + BONUS"
