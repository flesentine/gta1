extends "res://scripts/mission_director_build17.gd"

func _start_mission() -> void:
    if str(current_mission.get("id", "")) != "eastbound":
        super._start_mission()
        return
    if chain_points.is_empty():
        mission_state = "cooldown"
        mission_cooldown = 2.0
        _set_game_message("MISSION FAILED — HARBOR ROUTE MISSING", 2.0)
        return
    chain_index = 0
    mission_timer = time_limit
    mission_state = "chain_steal"
    _spawn_target_vehicle()
    _set_game_message("EASTBOUND — STEAL THE BLUE HARBOR CAR", 2.4)
...
func _complete_mission() -> void:
    ...
    _save_progress()
    _set_game_message("EASTBOUND COMPLETE — HARBOR EAST REACHED  +%d" % reward, 3.2)

func _show_unlock_overlay() -> void:
    if completion_panel == null or completion_label == null:
        return
    completion_label.text = "LEVEL COMPLETE\nCENTRAL DISTRICT CLEARED\n\n5 POST-CLEAR JOBS UNLOCKED\nCROSSTOWN · DEAD DROP · RED FLAG · CROSSROADS · EASTBOUND\n\nHARBOR EAST NOW OPEN\n\nSCORE %07d   BEST %07d   x%d" % [
...
func _objective_text() -> String:
    if str(current_mission.get("id", "")) == "eastbound":
        if mission_state == "chain_steal":
            return "STEAL THE BLUE HARBOR CAR"
        if mission_state == "chain_drive":
            return "HARBOR CHECKPOINT %d/%d — SLOW BELOW %d" % [
...
func _refresh_hud() -> void:
    ...
    if game != null and game.hud_label != null:
        game.hud_label.text = game.hud_label.text.replace("BUILD 17", "BUILD 18")
    if hud_label != null:
        hud_label.text = hud_label.text.replace("BUILD 17", "BUILD 18")
        hud_label.text = hud_label.text.replace("4 POST-CLEAR JOBS UNLOCKED", "5 POST-CLEAR JOBS UNLOCKED")
