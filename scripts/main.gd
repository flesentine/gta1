...
const RESPAWN_POINT := Vector2(60, 80)
...
func _spawn_traffic() -> void:
    ...
    for i in range(spawn_plan.size()):
        ...
        car.configure_ai(
            traffic_routes[route_index], point_index,
...
func _spawn_pickups() -> void:
    var pickup_plan := [
        ...
        ["ammo", Vector2(-75, 82), 10], ["ammo", Vector2(82, -120), 10],
        ["ammo", Vector2(-900, -520), 10], ["ammo", Vector2(900, 520), 10],
        ["ammo", Vector2(900, -520), 10], ["ammo", Vector2(-900, 520), 10],
        ["bribe", Vector2(-1020, 0), 1], ["bribe", Vector2(1020, 650), 1],
        ["bribe", Vector2(0, -770), 1], ["bribe", Vector2(0, 770), 1]
    ]
...
func _sidewalk_routes() -> Array[PackedVector2Array]:
    ...
    var x_spans := [Vector2(-1540, -1040), Vector2(-760, -140), Vector2(140, 760), Vector2(1040, 1540)]
    var y_spans := [Vector2(-1140, -790), Vector2(-510, -140), Vector2(140, 510), Vector2(790, 1140)]
    var margin := 38.0
    for xs in x_spans:
        for ys in y_spans:
            routes.append(PackedVector2Array([
                Vector2(xs.x - margin, ys.x - margin), Vector2(xs.y + margin, ys.x - margin),
                Vector2(xs.y + margin, ys.y + margin), Vector2(xs.x - margin, ys.y + margin)
            ]))
    return routes
...
func _process(delta: float) -> void:
    ...
    respray_cooldown = max(respray_cooldown - delta, 0.0)
    status_message_timer = max(status_message_timer - delta, 0.0)
    ...
    _update_police()
    _check_police_consequences(delta)
    if respawn_timer > 0.0:
        return
...
func _check_pickups() -> void:
    ...
    for pickup in pickups.duplicate():
        ...
        if pickup.pickup_kind == "pistol":
            ...
            pistol_ammo += pickup.amount
        elif pickup.pickup_kind == "bribe":
            if wanted_level <= 0:
                continue
            wanted_level = maxi(wanted_level - pickup.amount, 0)
            wanted_decay_timer = 7.0 if wanted_level > 0 else 0.0
            status_message = "POLICE BRIBE  -1 WANTED"
...
func _check_respray() -> void:
    if not in_vehicle or wanted_level <= 0 or respray_cooldown > 0.0:
        return
    if not is_instance_valid(current_vehicle):
        return
    if not RESPRAY_RECT.has_point(current_vehicle.global_position):
        return
    if current_vehicle.get_forward_speed_abs() > 85.0:
        return
    wanted_level = 0
    wanted_decay_timer = 0.0
    respray_cooldown = 2.0
    status_message = "RES-PRAYED — WANTED CLEARED"
    status_message_timer = 2.0
    _clear_police()
...
func _raise_wanted(amount: int) -> void:
    wanted_level = clampi(wanted_level + amount, 0, 4)
...
func _update_wanted(delta: float) -> void:
    ...
    if nearest_police <= 620.0:
        ...
        if wanted_decay_timer <= 0.0:
            wanted_level = maxi(wanted_level - 1, 0)
...
func _update_police() -> void:
    ...
    if wanted_level >= 3:
        desired_count += 1
    desired_count = mini(desired_count, 5)
...
func _check_police_consequences(delta: float) -> void:
    if wanted_level <= 0:
        arrest_progress = 0.0
        return
    if in_vehicle:
        arrest_progress = 0.0
        return
    var nearest_cop_distance := INF
    for cop in police:
        if not is_instance_valid(cop):
            continue
        var distance := player.global_position.distance_to(cop.global_position)
        nearest_cop_distance = min(nearest_cop_distance, distance)
        if distance <= 44.0 and cop.get_forward_speed_abs() >= 170.0:
            _lose_life("WASTED")
            return
    if nearest_cop_distance <= 54.0:
        arrest_progress += delta
        if arrest_progress >= 1.15:
            _lose_life("BUSTED")
    else:
        arrest_progress = max(arrest_progress - delta * 2.0, 0.0)
...
func _finish_respawn() -> void:
    ...
    player.visible = true
    player.set_active(true)
    status_message = "BACK ON THE STREET"
...
func _draw() -> void:
    draw_rect(RESPRAY_RECT, Color(0.52, 0.12, 0.48, 0.30), true)
    draw_rect(RESPRAY_RECT, Color(0.94, 0.35, 0.82, 0.85), false, 5.0)
...
func _update_hud() -> void:
    ...
    if not in_vehicle:
        ...
        if RESPRAY_RECT.has_point(current_vehicle.global_position) and wanted_level > 0:
    ...
    if arrest_progress > 0.0:
        arrest_text = "   BUST %.0f%%" % clamp(arrest_progress / 1.15 * 100.0, 0.0, 100.0)
    var message_text := ""
    if status_message_timer > 0.0 or respawn_timer > 0.0:
        message_text = "\n*** %s ***" % status_message
    hud_label.text = "GTA1 REMAKE — BUILD 6\n%s%s\n%s   WANTED %s%s\nLIVES %d   TRAFFIC %02d   PEDS %02d   POLICE %02d%s" % [
        mode, extra, weapon_text, _wanted_text(), arrest_text, lives,
        max(vehicles.size() - 1, 0), max(pedestrians.size() - down_count, 0), police.size(), message_text
    ]
    help_label.text = "WASD / Arrows: move or drive   E: enter/exit   Space/F: fire   Green diamond: bribe   Pink bay: respray   R: reset"
